...
public class TreeViewAdapter extends BaseAdapter {
	...
	public View firstChildView;
	
	public TreeViewAdapter(Context context,
			List<ReportpaitElement> mfilelist,int index) {
		mInflater = LayoutInflater.from(context);
		for(int i=0;i<mfilelist.size();i++){
			if(mfilelist.get(i).getOutlineTitle().equals("报告分类")){
				//mfilelist.add(mfilelist.remove(i));
				mfilelist.remove(i);
				i--;
			}
		}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(firstChildView == null){
			firstChildView = convertView;
		}
		//if(mfilelist.get(position).getOutlineTitle().equals("报告分类"))
