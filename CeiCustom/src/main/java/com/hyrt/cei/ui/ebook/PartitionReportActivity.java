				...
				leftList.setOnItemClickListener(new OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> arg0, View arg1,
						...
						if (leftData.get(arg2).isExpanded()) {
							...
							/**
							 * 如果点击父级条目时，则从集合中找到所有它的子栏目，然后进行排序
							 */
							...
							for (ReportpaitElement reportElement : allData) {
								if (reportElement.getParent().equals(
									...
									temp.add(reportElement);
								}
							}
							//获取排序好的子集栏目 temp1
							ReportpaitElement[] reportpaitElements = new ReportpaitElement[temp.size()];
							for(int i=0;i<temp.size();i++){
								reportpaitElements[compareNumStr.get(temp.get(i).getOutlineTitle())-1] = temp.get(i);
							}
							ArrayList<ReportpaitElement> temp1 = new ArrayList<ReportpaitElement>();
							for(int i=0;i<reportpaitElements.length;i++){
								temp1.add(reportpaitElements[i]);
							}
							
							if(temp1.size() > 0){
								leftData.addAll(arg2 + 1, temp1);
								treeViewAdapter = new TreeViewAdapter(
										PartitionReportActivity.this, leftData, -1);
								leftList.setAdapter(treeViewAdapter);
							}
	...
	private void addExpLeftData() {
		...
		if (leftData.get(1).isMhasChild()) {
			...
			new Handler().postDelayed(new Runnable() {
				public void run() {
					View v = PartitionReportActivity.this.findViewById(R.id.yjbg_feilei_list_left);
			...
			new Handler().postDelayed(new Runnable() {
				public void run() {
					View v = PartitionReportActivity.this.findViewById(R.id.yjbg_feilei_list_left);
			         float x = (v.getLeft() + v.getRight()) / 2+20;
			         //float y = (v.getTop() + v.getBottom()) / 2;
			         float y = (v.getTop()+50);
			         MotionEvent down = MotionEvent.obtain(0, 0,
			         MotionEvent.ACTION_DOWN, x, y, 0, 0, 0, 0, 0, 0, 0);
